...
#define TURN_LEFT_FOLLOW 1
#define TURN_RIGHT_FOLLOW 2
#define GO_TO_TARGET 3
...
int main()
{
    ...
    //initial position angle of the pioneer
    float initialAngle;
  ...
  //setting the initial position angle to target
  point2 = (float*) wb_gps_get_values(centerGPS);
  initialAngle = atan2(targetZ-point2[2],targetX-point2[0]);
    ...
    */
    ...
    if(obstacle > 5.0)
    {
      ...
      if(deltaObstacle > TOLERANCE) 
        mode = TURN_RIGHT_FOLLOW;
      else 
        mode = TURN_LEFT_FOLLOW;
    }else if(obstacle<1.0 && fabs(initialAngle-targetAngle)<0.2) 
      mode = GO_TO_TARGET;
    
    printf("inital:%2f , taget:%2f",initialAngle,targetAngle);
    switch(mode){
      case TURN_LEFT_FOLLOW:
        printf("TURN_LEFT_FOLLOW \n");
        if(obstacle > 3.0)
        {
        ...
        break;
      case TURN_RIGHT_FOLLOW:
        printf("TURN_RIGHT_FOLLOW \n");
        if(obstacle > 3.0)
        {
          obstacle = 1.0 - obstacle * invMaxRangeTimesWidth;// compute the relevant overall quantity of obstacle
          speedFactor = (obstacle > OBSTACLE_THRESHOLD) ? 0.0 : SLOWDOWN_FACTOR;
